...
void OnRadioCadDone( bool channelActivityDetected )
{
	...
	{
        LoRaCad.Cad_Detect = true;

		DEBUG(2,"Cad_State3333 = Cad_Detect \r\n");
	}else
	{
        LoRaCad.Cad_Done = true;
...
static void OnRadioTxDone( void )
{
   DEBUG(2,"TxDone Radio.GetStatus( )---111 = %d\r\n",Radio.GetStatus( ));  
   ...
   if( LoRaMacDeviceClass != CLASS_C )
    ...
    {
    	DEBUG(3, "IsRxWindowsEnabled is true\r\n");
    ...
    else
    {
        ...
        DEBUG(2, "IsRxWindowsEnabled is false MacDone : %d\r\n",LoRaMacFlags.Bits.MacDone);
    }
...
static void PrepareRxDoneAbort( void )
{
    ...
    DEBUG(2,"RxSlot : %d, LoRaMacDeviceClass :%d\r\n",RxSlot,LoRaMacDeviceClass);
...
//    uint8_t data[14] = {0x80,0x50,0x65,0x87,0x78,0x00,0x0c,0x00,0x0c,0xeb,0xa3,0x14,0x97,0x9a}; //8050658778000c000ceba314979a
//    address = 0x78876550;
//    LoRaMacComputeMic( data, 14, nwkSKey, address, DOWN_LINK, 12, &mic );///MICÐ£Ñé: Ð£Ñémac+paload----MICÖ®Ç°Êý¾Ý
//    DEBUG(2,"--------mic --- %02x, micRx --- %02x\r\n",mic, micRx);
    ...
    switch( macHdr.Bits.MType )
    {
		...
		case FRAME_TYPE_DATA_CONFIRMED_UP:  ///Ôö¼Ó½Úµã¼äÍ¨ÐÅÊý¾Ý½ÓÊÕ´¦Àí		
        case FRAME_TYPE_DATA_CONFIRMED_DOWN:
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
            {					
                address = payload[pktHeaderLen];
                LoRapp_SenSor_States.Node_ID[3] = payload[pktHeaderLen++];
                address |= ( (uint32_t)payload[pktHeaderLen] << 8 );
                LoRapp_SenSor_States.Node_ID[2] = payload[pktHeaderLen++];
                address |= ( (uint32_t)payload[pktHeaderLen] << 16 );
                LoRapp_SenSor_States.Node_ID[1] = payload[pktHeaderLen++];
                address |= ( (uint32_t)payload[pktHeaderLen] << 24 );
                LoRapp_SenSor_States.Node_ID[0] = payload[pktHeaderLen++];

				DEBUG(2,"FRAME_TYPE_DATA_UNCONFIRMED_DOWN %08x, %08x, %02x\r\n",address, LoRaMacDevAddr,macHdr.Value);
                ...
                if( address != LoRaMacDevAddr )
                {
                    ...
                    LoRaMacDevAddr = address; ///·¢ËÍÊý¾ÝID=½ÓÊÕÊý¾ÝID
...
//                            appSKey = curMulticastParams->AppSKey;
                    ...
                    if( multicast == 0 )
                    {
                        ...
                        PrepareRxDoneAbort( );
						printf("RxDoneAbort return\r\n");
                        return;
                    }
                }

                fCtrl.Value = payload[pktHeaderLen++]; ///FCtrl bits: 0x20±íÊ¾ÎªACK
                
				DEBUG(2,"pktHeaderLen = %d %2x\r\n",pktHeaderLen,fCtrl.Value);
                
                if(macHdr.Value == 0x80)
                Send_Ack = true;

                sequenceCounter = ( uint16_t )payload[pktHeaderLen++];     ///½ÓÊÕµ½Êý¾ÝÖ¡ºÅ            
                sequenceCounter |= ( uint16_t )payload[pktHeaderLen++] << 8;	
                LoRapp_SenSor_States.Node_Seq = sequenceCounter;

                appPayloadStartIndex = 8 + fCtrl.Bits.FOptsLen;
				DEBUG(2,"½ÓÊÕÖ¡ºÅ = %d downLinkCounter = %d appPayloadStartIndex = %d\r\n ", sequenceCounter,downLinkCounter,appPayloadStartIndex);
				...
				sequenceCounterPrev = downLinkCounter; ///±£ÁôÉÏ´Î½ÓÊÕµ½µÄÖ¡ºÅ£ºÖ»½ÓÊÕÒ»¸öÖÕ¶ËÉè±¸ÊÇÕý³£µÄ
                				
                if(sequenceCounter < sequenceCounterPrev) ///µ±½ÓÊÕµ½¶à¸öÖÕ¶Ë·¢ËÍµ½µÄÊý¾ÝÊ±±ØÐëÖØÖÃÉÏ´Î½ÓÊÕÖ¡ºÅ£¬·ñÔò»á³öÏÖÒì³£MIC½âÃÜÊ§°Ü
				{      	
                	sequenceCounterPrev = downLinkCounter = 1;  				
				}
				
                sequenceCounterDiff = ( sequenceCounter - sequenceCounterPrev );  ///¶ª°üÊý
				
				DEBUG(2,"sequenceCounterPrev = %d sequenceCounter = %d sequenceCounterDiff = %d %d\r\n",sequenceCounterPrev,sequenceCounter,sequenceCounter - sequenceCounterPrev, 1 << 15);

                if( sequenceCounterDiff < ( 1 << 15 ) ) ///:16Î»
                {
                    downLinkCounter += sequenceCounterDiff; ///++1
                    DEBUG(2,"DOWN_LINK = %d downLinkCounter = %d \r\n",DOWN_LINK,downLinkCounter);
                    DEBUG(2,"adress %08x, len %d\r\n",address, size - LORAMAC_MFR_LEN);
                    
                    for(uint8_t i = 0; i <16; i++)
                    DEBUG(2,"%02x",nwkSKey[i]);
                    DEBUG(2,"\r\n");
                    LoRaMacComputeMic( payload, size - LORAMAC_MFR_LEN, nwkSKey, address, DOWN_LINK, downLinkCounter, &mic );///MICÐ£Ñé: Ð£Ñémac+paload----MICÖ®Ç°Êý¾Ý
					DEBUG(2,"mic --- %02x, micRx --- %02x\r\n",mic, micRx);
                ...
                DEBUG(2,"bufferr size: %d--",size);
                for(uint8_t i = 0; i < size; i++)
                DEBUG(2,"%02x", payload[i]);
                DEBUG(2,"\r\n");
                ...
                if( isMicOk == true )
                {
                    ...
                    if( ( ( size - 4 ) - appPayloadStartIndex ) > 0 )
                    {
						...
						DEBUG(2,"%02x",McpsIndication.Buffer[i]);
						DEBUG(2,"BufferSize = %d\r\n",McpsIndication.BufferSize);                       
                    }
                    LoRaMacFlags.Bits.McpsInd = 1;
                }
                else
...
//                    downLinkCounter --;
                    ...
                    DEBUG(2,"INFO_STATUS_MIC_FAIL mic = %02x, DownLinkCounter = %d, %d\r\n",mic,curMulticastParams->DownLinkCounter,DownLinkCounter);
				//	AddMacCommand( MOTE_MAC_LINK_ADR_ANS, 0x07, snr ); ///×ö¼ÙÉèÈç¹ûÍø¹Ø·¢ËÍADR_req,½ÚµãÃ»ÉÏ·¢ADR_ANSÔò»áµ¼ÖÂÍø¹Ø²»¶ÏÏÂ·¢Êý¾Ý£¬¼ÙÉè³ÉÁ¢
					//µ±MICÕýÈ·ºó£¬·þÎñÆ÷Ôò²»ÔÙÏÔÊ¾ÖÕ¶ËANS´íÎó
					PrepareRxDoneAbort( );
                    return;
                }
            }
    ...
    // Trig OnMacCheckTimerEvent call as soon as possible
    TimerSetValue( &MacStateCheckTimer, 1 ); //10ms
    TimerStart( &MacStateCheckTimer );
}

static void OnRadioTxTimeout( void )
{
    DEBUG(2,"OnRadioTxTimeout\r\n");
    if( LoRaMacDeviceClass != CLASS_C )
    {
        Radio.Sleep( );
		Radio.TxSleep( );
...
static void OnMacStateCheckTimerEvent( void ) ///LoRaMacPrimitives->MacMcpsIndication( &McpsIndication );å®šæ—¶æ£€æŸ¥macå±‚
{
    TimerStop( &MacStateCheckTimer );
    bool txTimeout = false;
	
	DEBUG(3, "%s MacDone : %d\r\n",__func__, LoRaMacFlags.Bits.MacDone);
    ...
    if( LoRaMacState == MAC_IDLE )
    {
        ...
        if( LoRaMacFlags.Bits.MlmeReq == 1 )
        {
            DEBUG(2,"LoRaMacFlags.Bits.MlmeReq111\r\n");
    ...
    if( LoRaMacFlags.Bits.McpsInd == 1 )
    {
        DEBUG(2,"LoRaMacFlags.Bits.MlmeReq222\r\n");
...
void OnRxWindow1TimerEvent( void )
{
    ...
    ///RX1 > TX
	if(Channel == 7 || Channel == 15)///Æµ¶Î¶Ï¿ª´¦Àí·½Ê½
	{
		uint8_t savechannel = Channel;
		
		RxWindowSetup( (Channels[++savechannel].Frequency), datarate, bandwidth, symbTimeout, false );//false---trueÁ¬Ðø½ÓÊÕ
        LoRapp_SenSor_States.Freq_Rx1 = (Channels[++savechannel].Frequency)/10000;  
    }
    else
    {
        RxWindowSetup( (Channels[Channel].Frequency + 200000), datarate, bandwidth, symbTimeout, false );//false---trueÁ¬Ðø½ÓÊÕ
        LoRapp_SenSor_States.Freq_Rx1 = (Channels[Channel].Frequency)/10000;  
    }
...
//        RxWindowSetup( Rx2Channel.Frequency, Rx2Channel.Datarate, bandwidth, symbTimeout, false ); 
		  RxWindowSetup( (Channels[Channel].Frequency), datarate, bandwidth, symbTimeout, false); ///Ç¿ÖÆÇÐ»»
          LoRapp_SenSor_States.Freq_Rx2 = (Channels[Channel].Frequency)/10000;
    }
    else
    {
///add buy Ysheng.
        ...
        if(Channel>22)
        {
            DEBUG(2,"RX2Frequency is Error\r\n");
        }
        else
        {           
            if( Channel == 0 )///Æµ¶Î¶Ï¿ª´¦Àí·½Ê½
            {                    
                RxWindowSetup( (Channels[Channel].Frequency - 200e3), datarate, bandwidth, symbTimeout, false );//false---trueÁ¬Ðø½ÓÊÕ
                DEBUG(2,"RX2Frequency = %f, Datarate = %d\r\n",(Channels[Channel].Frequency - 200e3),datarate);  
                LoRapp_SenSor_States.Freq_Rx2 = (Channels[Channel].Frequency)/10000;
            }
            else
            {
                uint8_t savechannel = Channel;
                RxWindowSetup( (Channels[--savechannel].Frequency), datarate, bandwidth, symbTimeout, true );
                DEBUG(2,"RX2Frequency = %d, Datarate = %d\r\n",(Channels[savechannel].Frequency),datarate);  
                LoRapp_SenSor_States.Freq_Rx2 = (Channels[savechannel].Frequency)/10000;
            }   
        }
...
static void SetPublicNetwork( bool enable )
{
    ...
    Radio.TxSetModem( MODEM_LORA );
    if( PublicNetwork == true )
    {
        // Change LoRa modem SyncWord
        Radio.Write( REG_LR_SYNCWORD, LORA_MAC_PUBLIC_SYNCWORD );
        Radio.TxWrite( REG_LR_SYNCWORD, LORA_MAC_PUBLIC_SYNCWORD );
    }
    else
    {
        // Change LoRa modem SyncWord
        Radio.Write( REG_LR_SYNCWORD, LORA_MAC_PRIVATE_SYNCWORD );
        Radio.TxWrite( REG_LR_SYNCWORD, LORA_MAC_PRIVATE_SYNCWORD );
    }
}
...
static void RxWindowSetup( uint32_t freq, int8_t datarate, uint32_t bandwidth, uint16_t timeout, bool rxContinuous )
{
    ...
    DEBUG(2,"Radio.GetStatus( ) = %d\r\n",Radio.GetStatus( ));
    if( Radio.GetStatus( ) == RF_IDLE )
    {
        Radio.SetChannel( freq );
		DEBUG(2,"RxWindowSetup freq = %d\r\n",freq);
        ...
        else
        {
			...
			if(LoRaCad.Iq_Invert) ///µã¶Ôµã
			{
				 Radio.SetRxConfig( modem, bandwidth, downlinkDatarate, 1, 0, 10, timeout, false, 0, false, 0, 0, false, rxContinuous ); //false -- P2P
				 DEBUG(2,"Radio.SetRxConfig\r\n");
			}
			else
			{
				Radio.SetRxConfig( modem, bandwidth, downlinkDatarate, 1, 0, 10, timeout, false, 0, false, 0, 0, true, rxContinuous ); //true---GW
			}
...
static bool ValidatePayloadLength( uint8_t lenN, int8_t datarate, uint8_t fOptsLen )
{
    ...
    if( RepeaterSupport == true )
    {
		...
		LoRapp_SenSor_States.ADR_Datarate = datarate;
    }
    else
    {
        maxN = MaxPayloadOfDatarate[datarate];
		LoRapp_SenSor_States.ADR_Datarate = datarate; ///datarate»Øµ÷Ê¹ÓÃ
    }
...
LoRaMacStatus_t SendFrameOnChannel( ChannelParams_t channel )
{
    ...
    Radio.TxSetChannel( channel.Frequency  ); ///+ 200000
    ...
    { // Normal LoRa channel
        Radio.TxSetMaxPayloadLength( MODEM_LORA, LoRaMacBufferPktLen );
        Radio.SetTxConfig( MODEM_LORA, txPower, 0, 0, datarate, 1, 10, false, true, 0, 0, false, 3e3 );
        TxTimeOnAir = Radio.TimeOnAir( MODEM_LORA, LoRaMacBufferPktLen );
    }
#elif defined( USE_BAND_915 ) || defined( USE_BAND_915_HYBRID )
    Radio.SetMaxPayloadLength( MODEM_LORA, LoRaMacBufferPktLen );
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks )
{
    ...
    ChannelsDefaultDatarate = ChannelsDatarate = LoRapp_SenSor_States.default_datarate = 1; //LORAMAC_DEFAULT_DATARATE;
		
    DEBUG(2,"RF_Send_Data = %d\r\n",LoRapp_SenSor_States.default_datarate);
    ChannelsNbRep = 1;
    ChannelsNbRepCounter = 0;
...
LoRaMacStatus_t LoRaMacMulticastChannelLink( MulticastParams_t *channelParam )
{
    ...
    {
    	DEBUG(2,"MulticastChannels NULL\r\n");
        return LORAMAC_STATUS_PARAMETER_INVALID;
    }
    if( ( LoRaMacState & MAC_TX_RUNNING ) == MAC_TX_RUNNING )
    {
    	DEBUG(2,"LORAMAC_STATUS_BUSY NULL\r\n");
        return LORAMAC_STATUS_BUSY;
    }
