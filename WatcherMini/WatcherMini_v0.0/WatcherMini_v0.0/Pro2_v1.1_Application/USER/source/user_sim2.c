...
//	{NULL,				"CONNECT OK",			25000,			1,			AT_NONE},	// 连接服务器,具体服务器由用户指定
...
uint8_t SimCheckModle(void)
{
    ...
    gps_cmd.Command="AT+CPIN?\r\n";			
	gps_cmd.ExpectReply="+CPIN: READY";			
	gps_cmd.WaitTime=3000;
	gps_cmd.RetryCount=5;
	gps_cmd.AtRlplyState=AT_NONE;
	at_cmd[AT_CMD_USER]=gps_cmd;
    if(SimExecuteCmd(AT_CMD_USER)!=AT_OK)
    {
...
uint8_t SimInit(void)
{
    ...
    //等待模块初始化完成
	gps_cmd.Command="AT+CIURC=1\r\n";
	gps_cmd.ExpectReply="SMS Ready";			
	gps_cmd.WaitTime=120000;
	gps_cmd.RetryCount=1;
	gps_cmd.AtRlplyState=AT_NONE;
	at_cmd[AT_CMD_USER]=gps_cmd;
	if(SimExecuteCmd(AT_CMD_USER)!=AT_OK)
		return 0;
    ...
    //设置工作模式
	gps_cmd.Command="AT+CGCLASS=\"CC\"\r\n";			
	gps_cmd.ExpectReply="OK";			
	gps_cmd.WaitTime=2000;
	gps_cmd.RetryCount=2;
	gps_cmd.AtRlplyState=AT_NONE;
	at_cmd[AT_CMD_USER]=gps_cmd;
	if(SimExecuteCmd(AT_CMD_USER)!=AT_OK)//所有新的SIM模块都需要设置一次工作模式，否则会出现PDP DEACT
...
uint8_t SimConnectServer(char *ServerIP,char *ServerPort)
{
    ...
    gps_cmd.Command=command;			
	gps_cmd.ExpectReply="CONNECT OK";			
	gps_cmd.WaitTime=25000;
	gps_cmd.RetryCount=1;
	gps_cmd.AtRlplyState=AT_NONE;
	at_cmd[AT_CMD_USER]=gps_cmd;
	if(SimExecuteCmd(AT_CMD_USER)==AT_OK)
...
uint8_t SimSendData(char *data,uint16_t len)
{
	...
	if(SimExecuteCmd(AT_CMD_USER)==AT_OK)
	{
		//DEBUG("send:%s",temp);
		HAL_Delay(100);
		gps_cmd.Command=data;			
        gps_cmd.ExpectReply="SEND OK";			
        gps_cmd.WaitTime=2000;
        gps_cmd.RetryCount=1;
        gps_cmd.AtRlplyState=AT_NONE;
        at_cmd[AT_CMD_USER]=gps_cmd;
...
{
    T_AtCmdStruct gps_cmd;
    gps_cmd.Command="AT+CIPSHUT\r\n";			
	gps_cmd.ExpectReply="SHUT OK";			
	gps_cmd.WaitTime=5000;
	gps_cmd.RetryCount=1;
	gps_cmd.AtRlplyState=AT_NONE;
	at_cmd[AT_CMD_USER]=gps_cmd;
...
void SimCheckReply(void)
{
	...
	{
		at_cmd[AT_CMD_USER].AtRlplyState=AT_OK;
        G_IsSendData=1;
		SimReCount = 0;
	}
	...
	else if ( SimReCount>=4 && SimReBuff[SimReCount-2]=='\r' && SimReBuff[SimReCount-1]=='\n' )	// 收到"\r\n"，有新命令回复
	{
		...
		for (int8_t i=sizeof(at_cmd)/sizeof(at_cmd[0])-1; i>=0; i--)
		{
            ...
            {
                at_cmd[AT_CMD_USER].AtRlplyState=AT_ERROR;
            }
		}
