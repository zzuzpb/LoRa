...
uint8_t SernsorGetData(T_SensorData  *pt_sensor_data)
{
	...
	{
		DEBUG("读取光照度失败\r\n");
		pt_sensor_data[0].count=2;
		pt_sensor_data[0].data[0]=0;
		pt_sensor_data[0].data[1]=0;
		//return 0;
	}
	else
	{
		pt_sensor_data[0].count=2;
		pt_sensor_data[0].data[0]=(uint16_t)((lux&0xffff0000)>>16);
		pt_sensor_data[0].data[1]=(uint16_t)(lux&0x0000ffff);
	}
//	pt_sensor_data++;
	if(SensorHdc1080ReadData(sh)==0)
	{
		DEBUG("读取温湿度失败\r\n");
		pt_sensor_data[1].count=2;
		pt_sensor_data[1].data[0]=0;//温度
		pt_sensor_data[1].data[1]=0;//湿度
	}
	else
	{
		pt_sensor_data[1].count=2;
		pt_sensor_data[1].data[0]=(uint16_t)sh[0];//温度
		pt_sensor_data[1].data[1]=(uint16_t)sh[1];//湿度       
	}
	return 1;
}
/******************************************************读取空气温湿度***********************************************/
