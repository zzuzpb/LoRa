 ...
 * Channels datarate
 */
static int8_t ChannelsDatarate = 5; //LORAMAC_DEFAULT_DATARATE;

/*!
 * Channels default datarate
 */
static int8_t ChannelsDefaultDatarate = 5; //LORAMAC_DEFAULT_DATARATE;
...
static bool ValidatePayloadLength( uint8_t lenN, int8_t datarate, uint8_t fOptsLen )
{
    ...
    else
    {
        maxN = MaxPayloadOfDatarate[datarate];
				RF_Send_Data.ADR_Datarate = datarate; ///datarate回调使用
    }
