...
uint8_t device_type = BATTERY_DEVICE;//设备类型
...
void InitPower(void)
{
	...
	{
		HAL_GPIO_TogglePin(GPIOA,GPIO_PIN_15);	//PB9置0 
		delay_ms(200);
	}
	Bq24195EnableCharge( );	
	
#if SET_BQ24195   ///不使用软件配置充电IC，存在问题
	...
	state = SetHighImpedanceMode(DISABLE_CONTROL);		//清除高阻态状态	
...
void InitBq24195(void)
{
	...
	HAL_GPIO_WritePin(GPIOB,GPIO_PIN_9,GPIO_PIN_RESET);	//PB9置0 
		...
		//中断线4-PC15
//	HAL_NVIC_SetPriority(EXTI4_15_IRQn,1,0);       //抢占优先级为0，子优先级为0
