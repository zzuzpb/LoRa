...
水流量传感器：Q = 3600*f/k  f=脉冲频率  k=381.52  水流：1MIN/次 (m3/h  L/h)
...
GPS位置上报：特定按键触发时获取数据后上报一次(安装时使用)，服务器下行上报位置命令查询使用：预留接口后期使用

预留接口：
1：GPS位置上报：待定

2：唤醒触发时数据上报周期(最小时间必须限定在30S,增加休眠机制：不能是简单休眠，休眠过程会导致下行数据接收失败，此过程将再次使用CAD模式)
...
const int8_t TxPowers[]    = { 20, 14, 11,  8,  5,  2 };  //0 1 2 3 4 5


			//读取水流量

			///RF_Send_Data.Send_Buf = 电量+水流量+水压：数据解析运算在服务器进行,包括高低位调整
//				 for(uint8_t i = 0, j = 0; i < 6; i++, j++)
//				{							
//					RF_Send_Data.Send_Buf[i] = (Sensors_Data.temp[j]>>8)&0xff;
//					printf("%02x-%02x ",RF_Send_Data.Send_Buf[i],(Sensors_Data.temp[j]>>8)&0xff);
//					i++;
//					RF_Send_Data.Send_Buf[i] = Sensors_Data.temp[j]&0xff;
//					printf("%02x-%02x ",RF_Send_Data.Send_Buf[i],Sensors_Data.temp[j]&0xff);
//				}
//				printf("\r\n");
