...
int8_t ExcuteRs485Command(Rs485Command command)
{
	...
	HAL_Delay(20);//等待485模块切换为接收：去除会异常读取
	
	uint32_t uart_over_time = HAL_GetTick( ); ///串口查询不到数据保护机制，防止没接485下异常

	while( (HAL_GetTick( ) - UART_RX_DATA5.rxtime <= 10) || (HAL_GetTick( ) - uart_over_time < 100) ); //等待485接收数据完成：最多11Bit = 10ms//

	if(HAL_GetTick( ) - UART_RX_DATA5.rxtime > 10 && UART_RX_DATA5.USART_RX_Len != 0)//20msRX超时机制 
	{
