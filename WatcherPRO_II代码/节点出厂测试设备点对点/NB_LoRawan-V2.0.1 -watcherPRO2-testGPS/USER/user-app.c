...
#define APP_START_DUTYCYCLE                             (3000000)     // 3S
...
{	
	 ...
	 RF_Send_Data.ADR_Datarate = RF_Send_Data.default_datarate = Get_Flash_Datas.datarate = 2;
...
//	 Get_Flash_Datas.sleep_times = 5;
	DEBUG(3,"sleep_timeS = %d\r\n",Get_Flash_Datas.sleep_times);
...
//	LORAMAC_MAX_DATARATE = 5;  ///MAX DataRate：固定SF进行通信：最小SF：不开启ADR则不起作用
  ...
  { 
	...
	memcpy(DEV,&Devaddr[5],4);
    
    if(Devaddr[4] == 0x22) ///硬件存在GPS
    {
        LoRapp_SenSor_States.Hardware_Exist_GPS = true;
        DEBUG(3,"Hardware_Exist_GPS : \r\n");
    }
...
void OnReportTimerEvent( void )
{
    ...
    LoRapp_SenSor_States.loramac_evt_flag = 0;
    __enable_irq();
    TimerSetValue( &ReportTimer, 4000000 ); ///1s
...
{	 	      
	switch(sys_sta)
	{
		...
		{ 
			...
			{				
				memcpy(RF_Send_Data.RF_BUF, send_buf, RF_Send_Data.RX_LEN);   //MAC+PHY=56  MAC = 13  				RF_Send_Data.Error_count = 0;	
			}	
...
//						TimerStart( &ReportTimer );
					...
					if(RF_Send_Data.RF_Send == true)						
					{
                        ...
                        sensor_time = HAL_GetTick( ) - system_time;
                        sensor_time /= 1000;
                        if(Rs485Tag_UNKNOW) 
                        {
                        ...
                        SetRtcAlarm(RF_Send_Data.Sleep_times);///4S误差	
                        Into_Low_Power( );	
#endif
                            
//                            TimerStop( &ReportTimer );
//							TimerSetValue( &ReportTimer, 8000000 + randr( -APP_TX_DUTYCYCLE_RND, APP_TX_DUTYCYCLE_RND ) );
//							TimerStart( &ReportTimer );
						}									
...
{	
	 if(RF_Send_Data.Get_sensor && Set_Gps_Ack.GPS_DONE) ///获取485、光照、空气温湿度传感器数据  
	 {
		...
		for(uint8_t i = 0; i <= samples.socket_id; i++)
		{
			...
			DEBUG(3,"%02x",RF_Send_Data.Send_Buf[2+i]);
		}
		DEBUG(3,"socket_id = %d\r\n",samples.socket_id);
