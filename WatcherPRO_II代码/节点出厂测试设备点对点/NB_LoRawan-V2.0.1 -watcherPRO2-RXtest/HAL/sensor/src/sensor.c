...
int8_t QueryRs485Addr(void)
{
	...
	{
		DEBUG(3,"query addr success:%d\r\n",rs485Buf[3]);
		return SUCCESS;
	}

	DEBUG(3,"query addr fail:%d\r\n",rs485Buf[3]);
	return ERROR;
}
...
void SamplingData(void)
{
	...
	if (CheckBattery()<=3)					//电池电量低
	{
		...
		Into_Low_Power( ); 					//电池电量特别低，直接重置进入休眠
	}
    
    DEBUG(2,"bettry:%d;\r\n",RF_Send_Data.Battery);

	samples.fields[samples.count++] = ((RF_Send_Data.Rechargeing | RF_Send_Data.GPS) <<8) | RF_Send_Data.Battery;	//充电状态+电量
	lux = Max44009ReadLux(); 							//读取光照
	if (lux<0||lux>18800000)
		lux = 0;										//光照异常，置0
	if(sensors_char.Max44009_State)
	{
		samples.fields[samples.count++] = (uint16_t)(lux>>16);			//光照高位
		samples.fields[samples.count++] = (uint16_t)(lux&0xffff);		//光照低位
	}
	DEBUG(2,"lux:%d;\r\n",lux);
				...
				if (rs485Tag.sockets[i]) ///取保存传感器地址
				{
					...
					if (SUCCESS==ControlExtendBox(1<<(temp-1)))//只有打开控制接口成功之后才查询传感器
					{
						...
						while((SUCCESS!=ControlExtendBox((1<<(temp-1) & 0x00))) && (HAL_GetTick( ) - ExtendBox_Over_time < 5000));
						 ///while后：6707 8417 10127 11837 13547			
					}
					else
					{
						for(;i<=5;i++)
