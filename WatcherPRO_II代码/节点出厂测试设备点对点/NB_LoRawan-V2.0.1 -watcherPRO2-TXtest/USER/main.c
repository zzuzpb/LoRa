...
///vbat * (R49/(R49+R44)) = vadc

extern uint32_t systimecounter;
...
{	
   ...
   RF_Send_Data.Send_Buf = (uint8_t *)malloc(sizeof(uint8_t)*5); ///使用指针必须分配地址空间，否则会出现HardFault_Handler错误
...
//   GpsSendAgainTime(  );
   ...
   {		
    ...
    if(systimecounter>6000)
    {
         LoRapp_SenSor_States.loramac_evt_flag = 0;  
         RF_Send_Data.Send_Buf = (uint8_t *)"deves";
         RF_Send_Data.RX_LEN = 5;
         do
        {	                  
            if( user_app_send(UNCONFIRMED, RF_Send_Data.Send_Buf, RF_Send_Data.RX_LEN, 2) == 0 )
            {                               							 
                DEBUG(2,"Wait ACK app_send UpLinkCounter = %d\r\n", LoRaMacGetUpLinkCounter( ));                
            }
            else 
            {
                DEBUG(2,"app_send failed\r\n");
            }
            HAL_Delay(1000);
        }
