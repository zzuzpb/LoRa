...
{	
	...
	Get_Flash_Datas.sync_time = randr( 0, 5 ); ///非固定模式下：随机时隙								
	...
	{		
		...
		LoRaCad.Rssi[LoRaCad.Channel_Num]  = Radio.Rssi(MODEM_LORA); 
		LoRaCad.Channel_Scan[LoRaCad.Channel_Num] = Channel;		
...
void LoRa_Detect_Mode(void)
{
	...
	if(!LoRaCad.Cad_Detect && LoRapp_SenSor_States.Rx_States == RFWRITE )
	{
		...
		{	
			...
			TimerStop( &CadTimer );
			TimerSetValue( &CadTimer, 8000000 );
			TimerStart( &CadTimer );
		}
	}
}
