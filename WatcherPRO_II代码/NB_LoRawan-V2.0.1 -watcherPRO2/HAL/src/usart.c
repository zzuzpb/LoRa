...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
			...
			{			
			 ...
			 if(GLL_State == strcmp((char *)UART_RX_DATA2.USART_RX_BUF, "$PMTK010,002*2D"))
				...
				else if(GLL_State == strcmp((char *)UART_RX_DATA2.USART_RX_BUF, "$PMTK001,220,3*30"))
				{
					...
					Set_Gps_Ack.GPS_OVER_TIME = HAL_GetTick( );  ///记录GPS定位时间
				}

				if(UART_RX_DATA2.USART_RX_BUF[UART_RX_DATA2.USART_RX_Len-6] == 'A')
				{
					DEBUG(2,"GPS_TIME22 : %d\r\n",HAL_GetTick( ) - Set_Gps_Ack.Get_PATION_TIME);
                    Set_Gps_Ack.PATION_Counter++;
                    
                    if(Set_Gps_Ack.PATION_Counter>=30)
                    {
                        Set_Gps_Ack.Get_PATION = true;
                        GPS_Disable(  );
                        Set_Gps_Ack.PATION_Counter = 0;
                    }
					
					memcpy(Set_Gps_Ack.GLL, UART_RX_DATA2.USART_RX_BUF, UART_RX_DATA2.USART_RX_Len);
					Set_Gps_Ack.GLL[UART_RX_DATA2.USART_RX_Len++]='\r';
                    Set_Gps_Ack.GLL[UART_RX_DATA2.USART_RX_Len++]='\n';

					DEBUG(2,"Counter:%d,GLL--%s",Set_Gps_Ack.PATION_Counter,Set_Gps_Ack.GLL);
	...
	{		
        DEBUG(3,"Len -- %d ",UART_RX_DATA5.USART_RX_Len);
