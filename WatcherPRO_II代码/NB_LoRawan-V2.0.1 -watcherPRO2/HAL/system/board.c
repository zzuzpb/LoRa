...
{ 
 ...
 /* Disable the Peripheral */	
	...
	GPIO_InitStructure.Pin = 0x7F0F;  ///0x7F0F   0xFB0F--- UART1-PA10(RX) SPI 不设置为模拟输入   0xFF0F GPIO_PIN_All
	...
	HAL_GPIO_Init(GPIOA, &GPIO_InitStructure); 
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8|GPIO_PIN_15,GPIO_PIN_SET); ///SYSCLK	
	...
	///SX1278DIO、CH_CE不设置为模拟输入
	GPIO_InitStructure.Pin = 0xFD38;  ///0xFDFF  0x9DF7 0x9D38 0xFD38
	GPIO_InitStructure.Mode = GPIO_MODE_ANALOG;
	GPIO_InitStructure.Pull = GPIO_NOPULL;
	GPIO_InitStructure.Speed     = GPIO_SPEED_FREQ_HIGH;
	...
	HAL_GPIO_Init(GPIOB, &GPIO_InitStructure); 
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5,GPIO_PIN_SET); ///SYSCLK	
