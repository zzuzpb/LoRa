...
/************************射频发送数据状态判断************************/
LoRapp_State LoRapp_SenSor_States = {false, false, false, 0, RFWRITE, 0};

/************************设置RF数据包参数************************/
RF_Send RF_Send_Data = {0, 0, {0}, 0, 5, 0, 0, 0, 0, false, false}; 
		...
		if(flags->Bits.RxData == 1)
		{
			if(mac_callback_g!=NULL)
			{
                ...
                else if(RxInfo.buf[0] == 'G') ///重新获取GPS定位信息
                {
                  ...
                  Get_Gps_Ack.GPS_DONE = false;
                  Get_Gps_Ack.START = false;
                  Get_Gps_Ack.POS_FIX = false;
                  Get_Gps_Ack.GPGLL	= false;      
                  GpsInit(  );
                  GpsSet(  );	
                }	
            }
...
{	
	...
	RF_Send_Data.ADR_Datarate = RF_Send_Data.default_datarate = Get_Flash_Datas.datarate = 1;
