...
getRs485Status --- _rs485_getPinStatus --- InitRs485



PLUS_485工作流程说明：

1：第一次上电，遍历6个485接口_rs485_getPinStatus：使用广播方式：0xfe	0x03	0x04	0x00	0x00	0x00	0x00	0xf5	0x3c
等待应答数据：0xfe	0x03	0x04	0x00		0x00	0x00----第四位是0x00则说明接入485扩展盒，同时第5位代表哪些IO口扩展盒接入的485
若第四位数据非0，则说明此时该IO口没接入485扩展盒，若广播后没接收到485应答，则可能存在当前接入的设备不支持广播模式，需要将g_stu_rs485预存
485设备全部遍历一次，确认485是否接入正常读取。

2：非扩展盒模式下遍历g_stu_rs485结构体，根据广播应答地址与g_stu_rs485结构体数据预存地址匹配，然后保存到g_rs485Sendbuff发送结构体，同时写入flash

3：接入扩展盒模式下，分别打开扩展盒5个口广播，遍历g_stu_rs485结构体，根据广播应答地址与g_stu_rs485结构体数据预存地址匹配，然后保存到g_rs485Sendbuff发送结构体，同时写入flash
