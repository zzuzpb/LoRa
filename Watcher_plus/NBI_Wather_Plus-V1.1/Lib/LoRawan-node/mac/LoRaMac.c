    ...
    switch( macHdr.Bits.MType )
    {
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
            {					
                ...
                if( address != LoRaMacDevAddr ) ///Ö±½Ó´¦Àí¹ã²¥ID£¬·Ç¹ã²¥Ôò¹ýÂË£¬²»×ö´¦Àí£¬¹ã²¥ID£º FF-FF-FF-FF£¬Ö»ÓÃÓÚCÀà
                {
					if(LoRaMacDevAddr != 0xFFFFFFFF && LoRaCsma.Disturb) ///¸ÉÈÅÐÅºÅ£º´¥·¢ÖØ·¢»úÖÆ£¬¶ø·ÇÔÙ´ÎÉÏ´«
					{               
                  ...
                  ///  else if() send Done Ôò²»ÊÇÕìÌýÄ£Ê½£¬´¦ÓÚ½ÓÊÕÄ£Ê½½ÓÊÕµ½·ÇÍø¹ØÓ¦´ðÊý¾Ý£¬ÐèÒª¿ªÆôÖØ·¢±£»¤»úÖÆ
                    else if(LoRaMacDevAddr != 0xFFFFFFFF && !LoRaCsma.Disturb)
                    {
                        RF_Send_Data.Send_Counter ++;
                        
                        if(RF_Send_Data.Send_Counter <= 1)
...
//                               appSKey = curMulticastParams->AppSKey;
                ...
                else
                {
                    ...
                    nwkSKey = LoRaMacNwkSKey;
                    appSKey = LoRaMacAppSKey;
                    downLinkCounter = DownLinkCounter;  ///Ä¬ÈÏ¶¼ÊÇ0
                }
...
static void OnRadioRxTimeout( void )
{
    if( LoRaMacDeviceClass != CLASS_C )
    {
        ///Ö»Õë¶ÔRX1±êÖ¾Î»²Ù×÷£¬ÆäËü¹ýÂË²»Ê¹ÓÃ
		if(RF_Send_Data.OnRxWindow1)
		{
            DEBUG(2,"OnRadioRxTimeout ACK Fail\r\n");
			RF_Send_Data.OnRxWindow1 = false;
    ...
    if( LoRaMacFlags.Bits.MacDone == 1 )
    {
        ...
        if( ( AckTimeoutRetry == true ) && ( ( LoRaMacState & MAC_TX_DELAYED ) == 0 ) )
        {
            ...
            if( ( AckTimeoutRetriesCounter < AckTimeoutRetries ) && ( AckTimeoutRetriesCounter <= MAX_ACK_RETRIES ) )
            {
                ...
                /************************* add buy Jason ************************/                              
                if(RF_Send_Data.Send_again) ///½ÓÊÕÊý¾ÝÊ§°Ü£¬ÖØ·¢»úÖÆ
                {
                    // Sends the same frame again
                    ...
                    DEBUG(2,"TimeOnAir = %d\r\n", 10*TIMEONAIR + randr(-10, 10)*TIMEONAIR);                  
...
void OnRxWindow1TimerEvent( void )
{
    ...
    if(Channel>23)
    {
        DEBUG(2,"RxWindow1 Frequency is Error\r\n");
    }
    else
    {
        ...
        DEBUG(2,"RxWindow1 Frequency %d, datarate %d\r\n",(Channels[SaveChannel].Frequency),datarate);
    }
...
static bool ValidatePayloadLength( uint8_t lenN, int8_t datarate, uint8_t fOptsLen )
{
    ...
    if( RepeaterSupport == true )
    {
        maxN = MaxPayloadOfDatarateRepeater[datarate];
				RF_Send_Data.ADR_Datarate = datarate;
    }
    else
    {
        maxN = MaxPayloadOfDatarate[datarate];
				RF_Send_Data.ADR_Datarate = datarate; ///datarate»Øµ÷Ê¹ÓÃ
    }
...
//	DEBUG(2, "channel.Frequency = %d\r\n",channel.Frequency);
    ...
    if(Channel<=22)
    {
        ...
        DEBUG(2, "Txchannel.Frequency is Error\r\n");
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks )
{
    ...
    ChannelsDefaultDatarate = ChannelsDatarate = RF_Send_Data.default_datarate; //LORAMAC_DEFAULT_DATARATE;
