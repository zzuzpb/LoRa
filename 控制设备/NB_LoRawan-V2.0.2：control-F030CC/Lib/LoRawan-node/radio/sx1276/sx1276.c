...
uint32_t SX1276GetTimeOnAir( RadioModems_t modem, uint8_t pktLen )
{
    ...
    case MODEM_LORA:
        {
						if(sendwkup == false) ///第一次上电设置发送数据参数，获取空速
						{
							SX1276.Settings.LoRa.Bandwidth = 7;
							switch(RF_Send_Data.ADR_Datarate)
							{
    ...
    case MODEM_LORA:
        {
            ...
            //DIO0=CADDone   DIO1=CADDETECTED : DIO0响应较慢，改为DIO3触发 & RFLR_DIOMAPPING1_DIO2_MASK  ) | RFLR_DIOMAPPING1_DIO0_00 | RFLR_DIOMAPPING1_DIO2_00 
...
{
    // Set RESET pin to 0
    GpioWrite( GPIOB,GPIO_PIN_13,GPIO_PIN_RESET );
	
    // Wait 1 ms
    DelayMs( 1 );

    // Set SET pin to 1
    GpioWrite( GPIOB,GPIO_PIN_13,GPIO_PIN_SET );
